...
function opt = dec(fo,L,U)
    D = length(L);      %Dimensão do problema (número de variáveis)
    NP = 100;            %Número de indivíduos na população (Number Population)
    ...
    for i = 1:NP
        for j = 1:D
            x1(i,j) = normrnd((L(j)+U(j))/2,rand()*((U(j)-L(j))/2)); %Distribuição Normal e Desvio Aleatório            
            while (x1(i,j)==0)
                x1(i,j) = normrnd((L(j)+U(j))/2,rand()*((U(j)-L(j))/2)); %Distribuição Normal e Desvio Aleatório            
            end
